...
func isSmartProxyManagerError(state *httransform.LayerState) bool {
	if _, ok := state.ResponseHeaders.GetString("x-crawlera-error"); ok {
		return true
	}

	return isSmartProxyManagerResponseError(state)
}

func isSmartProxyManagerResponseError(state *httransform.LayerState) bool {
