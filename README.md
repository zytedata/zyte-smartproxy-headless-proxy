...
[![Build Status](https://travis-ci.org/zytegroup/zyte-proxy-headless-proxy.svg?branch=master)](https://travis-ci.org/zytegroup/zyte-proxy-headless-proxy)
[![Go Report Card](https://goreportcard.com/badge/github.com/zytegroup/zyte-proxy-headless-proxy)](https://goreportcard.com/report/github.com/zytegroup/zyte-proxy-headless-proxy)

Zyte Smart Proxy Manager Headless Proxy is a proxy whose main intent
is to help users with headless browsers to use
[Zyte Smart Proxy Manager](https://zyte.com/smart-proxy-manager). This
includes different implementations of headless browsers
such as [Splash](https://zyte.com/splash),
...
Manager without a need to build [Squid](http://www.squid-cache.org/) chains or
...
2. Zyte Smart Proxy Manager uses
   [custom headers as configuration](https://zyte.com/docs/zyte-proxy-proxy-api.html#request-headers).
...
3. Also, it is rather hard and complex to maintain best practices of using
   ...
   [support of Browser Profiles](https://zyte.com/docs/zyte-proxy-proxy-api.html#zyte-proxy-profile)
...
4. Zyte Smart Proxy Manager works best with browsers only with some
   preconditions which users have to repeat everytime: session usage, some
   recommended headers like `Referer` etc.
...
```console
$ brew install --HEAD https://raw.githubusercontent.com/zytegroup/zyte-proxy-headless-proxy/master/zyte-proxy-headless-proxy.rb
```
...
```console
$ git clone https://github.com/zytegroup/zyte-proxy-headless-proxy.git
...
```console
$ docker pull zytegroup/zyte-proxy-headless-proxy
```
...
Flags:
  ...
  -u, --zyte-proxy-host=ZYTE-PROXY-HOST
                             Hostname of Zyte Smart Proxy Manager. Default is
                             proxy.zyte.com.
  -o, --zyte-proxy-port=ZYTE-PROXY-PORT
                             Port of Zyte Smart Proxy Manager. Default is 8010.
  -v, --dont-verify-zyte-proxy-cert
                             Do not verify Zyte Smart Proxy Manager own
                             certificate.
  -x, --zyte-proxy-headers=HEADER ...
  ...
  -z, --direct-access-hostpath-regexps=DIRECT-ACCESS-HOSTPATH-REGEXPS ...
                             A list of regexps for hostpath for direct access,
                             bypassing Zyte Smart Proxy Manager.
...
requests to `proxy.zyte.com:8010` with API key `myapikey`.
...
|--------------------------------------------------------------------------------------------------|-----------------------------------------|------------------------------------------|-----------------------------------|----------------------|
| Run in debug/verbose mode.                                                                       | `ZYTE_PROXY_HEADLESS_DEBUG`             | `-d`, `--debug`                          | `debug`                           | `false`              |
| Which IP this tool should listen on (0.0.0.0 for all interfaces).                                | `ZYTE_PROXY_HEADLESS_BINDIP`            | `-b`, `--bind-ip`                        | `bind_ip`                         | `127.0.0.1`          |
| Which port this tool should listen.                                                              | `ZYTE_PROXY_HEADLESS_BINDPORT`          | `-p`, `--bind-port`                      | `bind_port`                       | 3128                 |
| Path to the configuration file.                                                                  | `ZYTE_PROXY_HEADLESS_CONFIG`            | `-c`, `--config`                         | -                                 |                      |
| API key of Zyte Smart Proxy Manager.                                                             | `ZYTE_PROXY_HEADLESS_APIKEY`            | `-a`, `--api-key`                        | `api_key`                         |                      |
| Hostname of Zyte Smart Proxy Manager.                                                            | `ZYTE_PROXY_HEADLESS_ZYTEPROXYHOST`     | `-u`, `--zyte-proxy-host`                | `zyte_proxy_host`                 | `proxy.zyte.com` |
| Port of Zyte Smart Proxy Manager.                                                                | `ZYTE_PROXY_HEADLESS_ZYTEPROXYPORT`     | `-o`, `--zyte-proxy-port`                | `zyte_proxy_port`                 | 8010                 |
| Do not verify Zyte Smart Proxy Manager own TLS certificate.                                      | `ZYTE_PROXY_HEADLESS_DONTVERIFY`        | `-v`, `--dont-verify-zyte-proxy-cert`    | `dont_verify_zyte_proxy_cert`     | `false`              |
| Path to own TLS CA certificate.                                                                  | `ZYTE_PROXY_HEADLESS_TLSCACERTPATH`     | `-l`, `--tls-ca-certificate`             | `tls_ca_certificate`              | <embeded>            |
| Path to own TLS private key.                                                                     | `ZYTE_PROXY_HEADLESS_TLSPRIVATEKEYPATH` | `-r`, `--tls-private-key`                | `tls_private_key`                 | <embeded>            |
| Disable automatic session management                                                             | `ZYTE_PROXY_HEADLESS_NOAUTOSESSIONS`    | `-t`, `--no-auto-sessions`               | `no_auto_sessions`                | `false`              |
| Maximal ammount of concurrent connections to process                                             | `ZYTE_PROXY_HEADLESS_CONCURRENCY`       | `-n`, `--concurrent-connections`         | `concurrent_connections`          | 0                    |
| Additional Zyte Smart Proxy Manager headers.                                                     | `ZYTE_PROXY_HEADLESS_ZYTEPROXYHEADERS`  | `-x`, `--zyte-proxy-headers`             | Section `zyte_proxy_headers`      |                      |
| Adblock-compatible filter lists.                                                                 | `ZYTE_PROXY_HEADLESS_ADBLOCKLISTS`      | `-k`, `--adblock-list`                   | `adblock_lists`                   |                      |
| Regular expressions for hostpath URL part for direct access, bypassing Zyte Smart Proxy Manager. | `ZYTE_PROXY_HEADLESS_DIRECTACCESS`      | `-z`, `--direct-access-hostpath-regexps` | `direct_access_hostpath_regexps`  |                      |
| Which IP should proxy API listen on (default is `bind-ip` value).                                | `ZYTE_PROXY_HEADLESS_PROXYAPIIP`        | `-m`, `--proxy-api-ip`                   | `proxy_api_ip`                    | <same as `bind_ip`>  |
| Which port proxy API should listen on.                                                           | `ZYTE_PROXY_HEADLESS_PROXYAPIPORT`      | `-w`, `--proxy-api-port`                 | `proxy_api_port`                  | 3130                 |
...
```console
$ zyte-proxy-headless-proxy -b 0.0.0.0 -p 3129 -u proxy.zyte.com -o 8010 -x profile=desktop -x cookies=disable
```
...
bind_ip = "0.0.0.0"
bind_port = 3129
zyte_proxy_host = "proxy.zyte.com"
zyte_proxy_port = 8010
...
* `zyte_proxy_requests` - a number of requests which were sent to Zyte Smart
     Proxy Manager.
...
* `zyte_proxy_errors` - a number of responses where `Zyte-Proxy-Error`
     header is set.
* `all_errors` - a number of responses with errors (canceled,
     timeouts and zyte_proxy_errors).
...
Please pay attention that usually requests_number and zyte_proxy_requests
are different. This is because headless proxy filters adblock requests
and also retries to recreate sessions which imply additional Zyte Smart Proxy
Manager requests. So, depending on the netloc proportion of these numbers can
differ.
...
Zyte Smart Proxy Manager is configured using the special headers, which have
`Zyte-Proxy-` as prefix. You can find a
[full list](https://zyte.com/docs/zyte-proxy-proxy-api.html#request-headers)
...
header is `Zyte-Proxy-Profile`. So, user can add following line to the
configuration:

```toml
[zyte_proxy_headers]
zyte-proxy-profile = "desktop"
```

or pass it via command line:

```console
$ zyte-proxy-headless-proxy ... -x zyte-proxy-profile=desktop
```

but it makes no sense to use `Zyte-Proxy-` prefix all the time, please omit it.
...
ZYTE_PROXY_HEADLESS_PROXY = "localhost:3128"

profile = webdriver.DesiredCapabilities.FIREFOX.copy()
profile["proxy"] = {
    "httpProxy": ZYTE_PROXY_HEADLESS_PROXY,
    "ftpProxy": ZYTE_PROXY_HEADLESS_PROXY,
    "sslProxy": ZYTE_PROXY_HEADLESS_PROXY,
    "noProxy": None,
    "proxyType": "MANUAL",
