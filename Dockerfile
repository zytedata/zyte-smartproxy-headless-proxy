...
ADD https://zyte.com/docs/_downloads/zyte-proxy-ca.crt /usr/local/share/ca-certificates/zyte-proxy-ca.crt
RUN set -x \
...
ENV ZYTE_PROXY_HEADLESS_BINDIP=0.0.0.0 \
    ZYTE_PROXY_HEADLESS_BINDPORT=3128 \
    ZYTE_PROXY_HEADLESS_PROXYAPIIP=0.0.0.0 \
    ZYTE_PROXY_HEADLESS_PROXYAPIPORT=3130 \
    ZYTE_PROXY_HEADLESS_CONFIG=/config.toml
EXPOSE 3128 3130
